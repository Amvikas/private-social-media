...
export const followUnfollowUser = async (req, res) => {
    try {
        ...
        const currentUser = await User.findById(req.user._id); // req.user is set in protectRoute middleware
        ...
        if (isFollowing) {
            ...
            // Follow
            await User.findByIdAndUpdate(id, {
                $push: { followers: req.user._id } // <-- FIXED HERE
            });
            await User.findByIdAndUpdate(req.user._id, {
                $push: { following: id }
            });
            // send notification to the user being followed (userToModify)
